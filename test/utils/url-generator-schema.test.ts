...
describe('URL Generator for schema queries', () => {
  ...
  it('generates a URL for a schema query', () => {
    const expected = 'http://sdw-wsrest.ecb.europa.eu/service/schema/dataflow'
      + '/ECB/EXR/1.0?explicitMeasure=false';
    const query = SchemaQuery.from({ context: 'dataflow', id: 'EXR', agency: 'ECB', version: '1.0' });
  ...
  it('offers to skip default values for schema', () => {
    ...
    const query = SchemaQuery.from({ context: 'dataflow', id: 'EXR', agency: 'ECB' });
  ...
  it('offers to skip defaults but adds them when needed (explicit)', () => {
    const expected = 'http://test.com/schema/dataflow/ECB/EXR?explicitMeasure=true';
    const query = SchemaQuery.from(
  ...
  it('supports metadataprovisionagreement since v2.0.0', () => {
    const expected = 'http://test.com/schema/metadataprovisionagreement/ECB/EXR?'
      + 'dimensionAtObservation=TEST';
    const query = SchemaQuery.from(
