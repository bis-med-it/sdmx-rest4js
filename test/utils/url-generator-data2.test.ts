...
describe('URL Generator for SDMX 3.0 data queries', () => {
  ...
  it('offers to skip default values (firstNObs)', () => {
    ...
    const query = DataQuery2.from({ context: 'dataflow=*:EXR(*)', firstNObs: 1 });
    const service = Service.from({ url: 'http://test.com', api: ApiVersion.v2_0_0 });
    const url = new UrlGenerator().getUrl(query, service, true);
    url.should.equal(expected);
  });
  ...
  it('offers to skip default values (lastNObs)', () => {
    ...
    const query = DataQuery2.from({ context: 'dataflow=*:EXR(*)', lastNObs: 2 });
    const service = Service.from({ url: 'http://test.com', api: ApiVersion.v2_0_0 });
    const url = new UrlGenerator().getUrl(query, service, true);
    url.should.equal(expected);
  });
  ...
  it('offers to skip default values (obsDim)', () => {
    ...
    const query = DataQuery2.from({ context: 'dataflow=*:EXR(*)', obsDimension: 'CURR' });
    const service = Service.from({ url: 'http://test.com', api: ApiVersion.v2_0_0 });
    const url = new UrlGenerator().getUrl(query, service, true);
    url.should.equal(expected);
  });
  ...
  it('offers to skip default values (attributes)', () => {
    ...
    const query = DataQuery2.from({ context: 'dataflow=*:EXR(*)', attributes: 'msd' });
    const service = Service.from({ url: 'http://test.com', api: ApiVersion.v2_0_0 });
    const url = new UrlGenerator().getUrl(query, service, true);
    url.should.equal(expected);
  });
  ...
  it('offers to skip default values (measures)', () => {
    ...
    const query = DataQuery2.from({ context: 'dataflow=*:EXR(*)', measures: 'none' });
    const service = Service.from({ url: 'http://test.com', api: ApiVersion.v2_0_0 });
    const url = new UrlGenerator().getUrl(query, service, true);
    url.should.equal(expected);
  });
  ...
  it('offers to skip default values (history)', () => {
    ...
    const query = DataQuery2.from({ context: 'dataflow=*:EXR(*)', history: true });
    const service = Service.from({ url: 'http://test.com', api: ApiVersion.v2_0_0 });
    const url = new UrlGenerator().getUrl(query, service, true);
    url.should.equal(expected);
  });
