...
import { AvailabilityQuery2Handler } from '../utils/url-generator-availability2';
...
class Generator {
  ...
  getUrl(query: any, service: any, skipDefaults?: boolean): string {
    ...
    if (this.query.context != null && this.query.attributes != null) {
      return new DataQuery2Handler().handle(
        this.query,
        this.service,
        skipDefaults,
      );
    } else if (this.query.context != null && this.query.mode != null) {
      return new AvailabilityQuery2Handler().handle(
        this.query,
        this.service,
        skipDefaults,
      );
    } else if (this.query.mode != null) {
      return new AvailabilityQueryHandler().handle(
        this.query,
        this.service,
        skipDefaults,
      );
    } else if (this.query.flow != null) {
      return new DataQueryHandler().handle(
        this.query,
        this.service,
        skipDefaults,
      );
    } else if (this.query.resource != null) {
      return new MetadataQueryHandler().handle(
        this.query,
        this.service,
        skipDefaults,
      );
    } else if (this.query.context != null) {
      return new SchemaQueryHandler().handle(
        this.query,
        this.service,
        skipDefaults,
      );
